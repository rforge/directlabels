...
\begin{frame}[fragile]
  ...
  Find a label position on a grid (grey rectangles)\\
  that is near the center of each point cloud (green dots), \\
  but does not overlap any points or other labels (black dots).
